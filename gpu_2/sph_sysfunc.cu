...
// {
//     cudaMemcpyToSymbol((char *)&dParam, hParam, sizeo0 mnf(SysParam));
// }
...
__global__
void integrate_velocity_kernel(Particle* dMem, uint num_particle, SysParam* dParam)
{
	uint index=blockIdx.x*blockDim.x+threadIdx.x;
...
void integrate_velocity(Particle *dMem, uint num_particle, SysParam* dParam)
{
	...
	integrate_velocity_kernel<<< num_block, num_thread >>>(dMem, num_particle, dParam);
}
