...
SPHSystem::SPHSystem()
{
  ...
  hParam->num=0;
	...
	alloc_array((void**)&(dBODY), sizeof(Particle)*hParam->max_body_particle);
...
//	printf("NumofParticle=%d\n", hParam->num_particle);
	...
	integrate_velocity(dMem, hParam->num_particle, dParam);
...
void SPHSystem::init_system()
{
	...
	void SPHSystem::init_boundary()
	{
		...
		for(pos.x=para.x*0.0f+kernel/2; pos.x<para.x*1.0f-kernel/2; pos.x+=(kernel*0.2f))
 	{
		...
		add_particle(pos, vel, hBC, hParam->num_particle_bc);
	}
 ...
 for(pos.y=para.y*0.0f+kernel/2; pos.y<para.y*1.0f-kernel/2; pos.y+=(kernel*0.2f))
 {
	...
	copy_array(dBC, hBC, sizeof(Particle)*hParam->num_particle_bc, CUDA_HOST_TO_DEV);
	copy_array(dParam, hParam, sizeof(SysParam), CUDA_HOST_TO_DEV);
