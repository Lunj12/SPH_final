...
int2 calc_grid_pos(float2 p)
{
    ...
    grid_pos.y = floor((p.y) / dParam.cell_size);
     ...
     printf("Flag0: Thread %d | dParam.kernel = %f | dParam.world_size = %f | dParam.cell_size = %f\n", threadIdx.x, dParam.kernel, dParam.world_size, dParam.cell_size);

    // printf("Thread %d | dParam.kernel = %f | dParam.world_size = %f | dParam.cell_size = %f\n", threadIdx.x, dParam.kernel, dParam.world_size, dParam.cell_size);
    //printf("Thread %d  p.y = %f\n | grid_pos.y = %f | dParam.cell_size = %f\n", threadIdx.x, p.y, grid_pos.y, dParam.cell_size);
...
void calc_hashK(uint *dHash, uint *dIndex, Particle *dMem, uint num_particle)
{
    ...
    // printf("Flag3a: dMem[8].pos.y = %f\n", dMem[8].pos.y);
    // printf("Thread %d | index = %d | dMem[index].pos.y = %f\n", threadIdx.x, index, dMem[index].pos.y);
    ...
    // printf("Thread %d gridx = %f dMem[index].pos.x = %f\n", threadIdx.x, grid_pos.x, dMem[index].pos.x);
    //printf("Thread %d | index = %d | dMem[index].pos.y = %f\n | grid_pos.y = %f | hash = %d\n", threadIdx.x, index, dMem[index].pos.y, grid_pos.y, hash);
    // printf("Thread %d hash = %d\n", threadIdx.x, hash);
...
void calc_hash(uint *dHash, uint *dIndex, Particle *dMem, uint num_particle)
{
	...
	Particle* hMem;
	hMem = (Particle*) malloc(sizeof(Particle)*20);
	cudaMemcpy(hMem, dMem, sizeof(Particle)*20, cudaMemcpyDeviceToHost);
	printf("Flag2a: hMem[8].pos.y = %f\n", hMem[8].pos.y);
	printf("Flag2b: hMem[16].pos.y = %f\n", hMem[16].pos.y);
